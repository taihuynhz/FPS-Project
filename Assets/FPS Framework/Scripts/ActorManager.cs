...
namespace FPSFramework
{
    ...
    public class ActorManager : MonoBehaviour
    {
        ...
        [Tooltip("The assigned prefap which will spwan on respwan")]
        public Actor actorPrefap;
        ...
        private void Start()
        {
            if (!actorPrefap) actorPrefap = GetComponentInChildren<Actor>();

            SpwanManager = SpawnManager.Instance;

            if (UIManager.Instance) UIManager.Instance.SetName(ActorName);

            if (randomizeName && randomNames.Count <= 0)
            {
                randomizeName = false;
                ActorName = $"{actor.type} {Random.Range(0, 1000)}";
        ...
        public Actor Spwan()
        {
            ...
            Actor newActor = Instantiate(actorPrefap, pos, rot, transform);
